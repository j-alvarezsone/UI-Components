...
import type { GenericOption, TooltipPlacement } from "../../type"
...
const { value, handleBlur, handleChange, errors, setTouched, setErrors } = useField<string>(() => props.name, undefined, {
...
const filteredOptions = computed(() => {
  if (!search.value || props.disableFilter)
...
async function updateDropdownPosition() {
  ...
  if (!containerRef.value || !containerProps.ref.value)
    return
...
function selectOption(optionValue: string, option: T) {
  if (props.disabled || toDisabled(option))
    return
...
const selectedOptionStyle = computed(() => {
  return [
    ...
    { "ring-3 ring-error-300/50 border-error-300": errors.value.length && !props.disabled && isDropdownVisible.value },
    { "border-error-500": errors.value.length && !props.disabled && !isDropdownVisible.value },
    // Text && Background
    ...
    <button type="button" :test-id="`${name}InputSelectButton`" :disabled="disabled" :class="selectedOptionStyle" @click="onToggleOpen">
      ...
      <template v-else>
        ...
        <div v-if="tooltip && !errors.length" v-tooltip="{ content: tooltip, placement: tooltipPlacement }" class="cursor-help order-2 shrink-0">
          <BaseIcon name="QuestionCircle" class="size-4 stroke-gray-400" />
        </div>
        ...
        <div class="chevron-icon" :class="dropDownIconStyle" @click.stop="onToggleOpen">
          <BaseIcon name="ChevronDown" :class="{ ' text-[#D0D5DD]': disabled }" />
        </div>
      </template>
    </button>
    <Teleport to="body">
