...
const { value, handleChange, handleBlur, errors } = useField<string>(() => props.name, undefined, {
    ...
    <div :class="style">
      ...
      <template v-else>
        ...
        <div v-if="errors.length && !disabled" class="grid place-content-center shrink-0 order-2">
          <BaseIcon name="ErrorOutline" class="size-4 text-error-500" fill-svg="fill-transparent" />
        </div>
        ...
        <div v-if="tooltip && !errors.length" v-tooltip="{ content: tooltip, placement: tooltipPlacement }" class="cursor-help grid place-content-center shrink-0">
          <BaseIcon name="QuestionCircle" class="size-4 stroke-gray-400" fill-svg="fill-transparent" />
        </div>
        ...
        <div v-if="value" class="cursor-pointer grid place-content-center shrink-0 size-4">
          <BaseIcon key="eye" :name=" mask ? 'EyeClosed' : 'Eye'" class="size-4 transition-opacity duration-300 ease-in-out hover:opacity-65" @click="onToggleMask" />
        </div>
      </template>
    </div>
