...
const { value, handleChange, handleBlur, errors, setErrors, setTouched } = useField<string>(() => props.name, undefined, {
    ...
    <div :class="style">
      ...
      <template v-else>
        ...
        <div v-if="errors.length && !disabled" class="grid place-content-center shrink-0 order-2">
          <BaseIcon name="ErrorOutline" class="size-4 text-error-500" />
        </div>
        <template v-else>
          <div
